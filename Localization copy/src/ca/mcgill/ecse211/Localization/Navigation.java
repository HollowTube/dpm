...
public class Navigation extends OdometerData implements UltrasonicController{
	...
	void travelTo(double x, double y){
		xyt = Lab4.odometer.getXYT();			//get current position
	...
	void turnToCoord(double x, double y){
		xyt = Lab4.odometer.getXYT();
		if(x >= 0 && y >= 0){										//angles are from the y-axis instead of x-axis
			turnTo((180/Math.PI)*Math.atan2(x-(Math.round(xyt[0]/Lab4.TILE_SIZE)), y-(Math.round(xyt[1]/Lab4.TILE_SIZE)))-xyt[2]);  		//angles in positive xy-plane
		}else if(y < 0){							
			turnTo(180+(180/Math.PI)*Math.atan2(x-(Math.round(xyt[0]/Lab4.TILE_SIZE)), y-(Math.round(xyt[1]/Lab4.TILE_SIZE)))-xyt[2]);	//angles in the negative y-plane
		}else{
			turnTo(360+(180/Math.PI)*Math.atan2(x-(Math.round(xyt[0]/Lab4.TILE_SIZE)), y-(Math.round(xyt[1]/Lab4.TILE_SIZE)))-xyt[2]); 	//angles in negative x-plane and positive y-plane
		}
...
//		if(distance < 15){
//			//usPoller will be constantly reading the distance from the sensor in unison with other thread
//			//if the distance falls below 15, the robot will slightly turn away and turn the US sensor as well
//			/*
//			 * The major issue with this method is that we have not implemented any means of communication
//			 * between this method and the Lab3 class. As it stands now, the robot will do its best to
//			 * avoid the obstacle but will then continue to the next coordinate point, it will not return to
//			 * the point it was trying to reach.
//			 * */
//			//int prevWp = Lab3.Lab3.wpCtr;
//			//spinMotor.setSpeed(50);
//			//spinMotor.rotate(-10);
...
//			leftMotor.setSpeed(FORWARD_SPEED);
//			rightMotor.setSpeed(FORWARD_SPEED);
//			leftMotor.rotate(convertDistance(Wheel_Radius, 20), true);	//travel straight
